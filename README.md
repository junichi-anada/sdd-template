...
7. テストの実行はエージェントが実行します（ローカルまたは CI）。
	- `make test` は PHPUnit に加えて `tests/*.sh` のシェルテストも自動実行し、結果を集約します（非実行権限の *.sh は警告してスキップ）。
...
### テスト実行（詳細）
- ローカル: `make test` は以下の順で実行します。
	1) PHPUnit（`composer.json` が存在する場合）
	2) シェルテスト: `tests/**/*.sh`（拡張子 .sh, 実行権限があるもの）
- 両者の結果をサマリし、どちらかが失敗すれば非ゼロで終了します。
- 例: `tests/meta_spec_generation.sh` はテンプレートのメタ仕様生成の回帰を検証します。
